...
Keep the existing horizontal wordmark in the header. Use `assets/logos/wildpath-circle-dark.svg` as the main footer logo and `wildpath-circle-light.svg` on light backgrounds or as a subtle watermark. Replace the current generic or repeated illustration placeholders with the matching packaged WebP files.
